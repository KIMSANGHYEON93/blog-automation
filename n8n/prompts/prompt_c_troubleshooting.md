...
5. **다이어그램**: 각 H2 섹션(FAQ 제외) 중 시각화가 유용한 곳에 ```mermaid 코드블록을 삽입하세요 (최소 2개). 다이어그램 유형은 내용에 따라 graph TD, sequenceDiagram, flowchart LR 등 자유롭게 선택하되, **노드 레이블은 영문**으로 작성하세요. <!-- IMAGE: --> 마커는 사용하지 마세요.
