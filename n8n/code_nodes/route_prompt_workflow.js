// Node 4: Route Prompt — 시트 데이터 + SERP 데이터 결합
...
// AI-Brain 볼트 용어 주입: 키워드와 매칭되는 내부 용어 정의를 프롬프트에 포함
let brainSection = '';
try {
  const brainRows = $('Sheets Read (Brain Terms)').all().map((i) => i.json);
  const kwLower = keyword.toLowerCase();
  const matched = brainRows.filter((r) => {
    const names = [r['용어'], ...String(r['별칭'] || '').split(',')]
      .map((s) => String(s).trim().toLowerCase())
      .filter((s) => s.length > 1);
    return names.some((n) => kwLower.includes(n) || n.includes(kwLower));
  }).slice(0, 3);
  if (matched.length > 0) {
  ...
  // 용어 노드가 없거나 조회 실패 시 주입 생략 (기존 동작 유지)
}
