...
4. **구조**: H2 → H3 → H4 헤딩을 논리적 트리 구조(Tree Structure)로 배치. H1은 사용 금지 (제목 필드에서 별도 관리). 에러 현상 → 원인 분석 → 해결 방법 (단계별) → 예방 조치 → 관련 에러/참고 자료 → FAQ
5. **LSI 최적화**: 메인 키워드(에러 코드)를 3회 이상 반복하지 마세요. 대신 문맥에 맞는 연관 키워드(증상, 원인 용어, 관련 에러 코드)를 본문 전체에 자연스럽게 분산 주입하세요
6. **체류 시간 설계**: 원인별 비교, 해결 방법 요약, 환경별 차이점 등은 반드시 마크다운 표(Table)로 시각화하여 독자 이탈률을 낮추세요
...
8. **코드블록 언어 태그**: 모든 코드블록에 반드시 언어 태그(```powershell, ```bash, ```yaml 등)를 붙이세요. 실행 전후 확인 명령어도 코드블록으로 감싸세요
...
| FAQ | 200자 | 3개 이상, 각 답변 80자 이상. **추천 스니펫 타겟팅**: 각 Q&A를 "~해결 방법은?", "~원인은?", "~방지하려면?" 등 검색 의도에 맞는 직관적 질문으로 작성하여 Google Featured Snippet에 노출되도록 최적화 |
...
```json
{
  ...
  "url_slug": "영문-하이픈-구분-슬러그 (예: 502-bad-gateway-nginx-fix)",
