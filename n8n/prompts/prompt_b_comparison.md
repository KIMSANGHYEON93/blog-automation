...
4. **구조**: H2 → H3 → H4 헤딩을 논리적 트리 구조(Tree Structure)로 배치. H1은 사용 금지 (제목 필드에서 별도 관리). 개요 → 각 기술 설명 → 심층 비교 분석 → 선택 가이드 → 마이그레이션/도입 시나리오 → FAQ
...
8. **코드 예시**: 각 기술의 설정/사용 예시 코드를 1개 이상 포함하세요. 코드블록에는 반드시 언어 태그(```yaml, ```bash 등)를 붙이세요
...
| FAQ | 200자 | 3개 이상, 각 답변 80자 이상. **추천 스니펫 타겟팅**: 각 Q&A를 "A와 B 차이점은?", "~선택 기준은?" 등 검색 의도에 맞는 직관적 질문으로 작성하여 Google Featured Snippet에 노출되도록 최적화 |
...
```json
{
  ...
  "url_slug": "영문-하이픈-구분-슬러그 (예: kubernetes-vs-docker-swarm-comparison)",
